...
extension FFKeyboardBar {
    ...
    private func addLbl() {
        ...
        for (i, title) in titles.enumerated() {
            ...
            // 计算X和W
            lblW = (title as NSString).boundingRect(with: CGSize(width: CGFloat(MAXFLOAT), height: 0), options: .usesLineFragmentOrigin, attributes: [NSAttributedString.Key.font:  lbl.font ?? style.normalFont], context: nil).width
